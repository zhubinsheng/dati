			...
			Ext.onReady(function(){
			    ...
			    questionsStore = Ext.create('Ext.data.Store', {
			        ...
			        proxy: {
			            ...
			            url: '<%=path%>/questionAction!getQuestionsList',
			            reader: {
			            	type: 'json',
			                root: 'root',
			    ...
			    questionsStore.on('beforeload', function (store, options) {
					...
					var new_params = {
						//query: Ext.getCmp('search').getValue()
						query	  	 : query,
			    ...
			    var tbar = Ext.create("Ext.toolbar.Toolbar", {
			    	items: [
			    		{text: '添加试题', 	iconCls: 'Add', 			id: 'add', 			handler: tbarFn}, 
			    			'-',
			    		{text: '删除试题', 	iconCls: 'Cross', 			id: 'del', 			handler: tbarFn},
			    			'-',
			    		{text: '编辑试题', 	iconCls: 'Cogedit', 		id: 'edit', 		handler: tbarFn},
			    			'-',
			    ...
			    function tbarFn(btn,pressed){
			    	if (btn.id == 'add'){
			    		...
			    		//显示添加对话框
			    		var url = "<%=path%>/questionAction!getQuestionAddOrEditPage";
			    		...
			    		addWindow.on("beforeclose",function(){
			    			questionsStore.load();
			    		});
			    		...
			    		if (treeNode == null || treeNode.length <= 0){
			    			Ext.MessageBox.alert("操作提示","您还未选择试题，请至少选择一道试题!");
			    			return ;
			    		}
			    		...
			    		Ext.Msg.show({
				    		...
				    		fn:function (btn,text){
				    			if(btn=="ok"){
						    		...
						    		$.ajax({
						    			url : '<%=path%>/questionAction!delQuestion',
						    			data : data,
						    			type : 'POST',
						    			dataType : 'json',
						    			success : function(jsonStr){
			    		...
			    		//获取节点数据
			    		var treeNode = questionsGrid.getSelectionModel().getLastSelected();
			    		if (treeNode == null){
			    			Ext.MessageBox.alert("操作提示","您还未选择试题，请选择一道试题!");
			    			return ;
			    		}
						
			    		//显示添加对话框
			    		var id 		 		= treeNode.get("id");
			    		var url = "<%=path%>/questionAction!getQuestionAddOrEditPage?id="+id;
			    		...
			    		editWindow.on("beforeclose",function(){
			    			questionsStore.load();
			    		});
			    		
			    		questionsGrid.getSelectionModel().clearSelections();
			    		questionsGrid.getView().refresh();
